...
import { EXCLUSIF_POST, GET_POSTS } from "../../constants/apiEndpoints";
import { FetchWrapper } from "../../helpers/apiRequest";
import { ChevronLeftIcon, ChevronRightIcon } from "@heroicons/react/outline";
import { Link } from "react-router-dom";

function ExclusifPost() {
  const [posts, setPosts] = useState([]);
  const httpRequest = FetchWrapper();

  useEffect(async () => {
    const response = await httpRequest.get(`${EXCLUSIF_POST}`);
    setPosts(response);
  }, []);

  const slideLeft = () => {
    var slider = document.getElementById("slider-exclusif");

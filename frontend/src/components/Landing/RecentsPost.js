...
import { RECENTS_POST } from "../../constants/apiEndpoints";
import { FetchWrapper } from "../../helpers/apiRequest";
import { Link } from "react-router-dom";

function RecentsPost() {
  const [posts, setPosts] = useState([]);
  const httpRequest = FetchWrapper();

  useEffect(async () => {
    const response = await httpRequest.get(`${RECENTS_POST}`);
    setPosts(response);
  }, []);

  const slideLeft = () => {
    var slider = document.getElementById("slider-recents");
